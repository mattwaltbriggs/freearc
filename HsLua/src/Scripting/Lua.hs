        ...
        if not v
            then return Nothing
                ...
                let getindex idx = do
                        ...
                        pop l 1
                        return v
                v <- mapM getindex (take len [1..])
                if all isJust v
                    then return (Just (map fromJust v))
                    else return Nothing
    valuetype _ = TTABLE
...
instance (StackValue a,StackValue b) => StackValue [(a,b)] where
    push l x = do
        ...
        zipWithM_ setindex [1..] x
    peek l n = do
        let n2 = if n<0 then n-1 else n
        let donext result = do
              g <- next l n2
              if g
                  then do
                      key <- peek l (-2)
                      val <- peek l (-1)
                      pop l 1
                      if isJust key && isJust val
                          then do
                              modifyIORef result ( (fromJust key,fromJust val) : )
                              donext result
                          else do
                              pop l 1 -- pop the key
                              return Nothing
...
register l n f = pushfunction l f >> setglobal l n
...
-- | Loads a Lua script given as string parameter and executes it immediatelly
-- | Loads a Lua script file and executes it immediatelly. Use as:
--
...
dostring l str = do
    ret <- loadstring l str "inline code"
    if ret/=0
        then return ret
        else call l 0 multret
