...
import Graphics.UI.Gtk.Abstract.Widget (keyReleaseEvent)
...
arcinfoDialog fm' exec mode arcnames arcdir files = do
  handle (\(_ :: SomeException) -> fmErrorMsg fm' "0013 There are no archives selected!") $ do
  fm <- val fm'
...
-----------------------------------------------------------------------------------------------
    ...
    labels <- foreach [0..1] $ \y -> do
      -- ������ �������
      label1 <- labelNew (Nothing :: Maybe String);  let x=0
    ...
    --
    for langFiles (New.comboBoxAppendText langComboBox . T.pack . mapHead toUpper . replace '_' ' ' . dropEnd 4 . drop 4)
    ...
    let getCurrentLangFile = do
          ...
          if lang >= 0
            then myCanonicalizePath (langDir </> (langFiles !! lang))
            else return ""

    -- ��� ������ ������� ����� ����������� ������� ���������� � ���
    GtkSigs.on langComboBox New.changed $ do
      lang <- New.comboBoxGetActive langComboBox
      when (lang >= 0) $ do
...
compressionPage fm' vbox = do
  ...
  ; methodText <- labelNew (Nothing :: Maybe String);                              boxPackStart hbox methodText       PackGrow 0
...
encryptionPage fm' dialog okButton vbox = do
  ...
  -- ��������� ������ OK ������ ���� ��� �������� ������ ���������
  let [pwd1,pwd2] = pwds
  for pwds $ \pw -> GtkSigs.after pw keyReleaseEvent $ do
    [pwd1', pwd2'] <- liftIO $ mapM val pwds
    liftIO $ okButton `widgetSetSensitivity` (pwd1'==pwd2')
    return False
